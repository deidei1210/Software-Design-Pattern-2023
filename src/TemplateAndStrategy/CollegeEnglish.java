...
public class CollegeEnglish extends Exam implements Preparation {
    public CollegeEnglish(String Name) {
        super(Name);
    }

    @Override
    public void examPreparation() {
        System.out.println("大学英语 模拟考试准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("大学英语 模拟考试准备完成！");
    }
    ...
    public void examStart() {
        ...
        System.out.println("大学英语 模拟考试开始！");
        for (int i = 0; i < 8; ++i) {
            getValue(i);
        }
        sortScore();
    ...
    public void examEnd() {
        ...
        System.out.println("大学英语 模拟考试结束！");
        System.out.println("大学英语模拟考试最终成绩：");
        setTaken(true);
        for (int i = 1; i <= 8; ++i) {
