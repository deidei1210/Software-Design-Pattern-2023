...
public class CollegePolitics extends Exam implements Preparation {
    public CollegePolitics(String Name) {
        super(Name);
    }

    @Override
    public void examPreparation() {
        System.out.println("大学政治 模拟考试准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("大学政治 模拟考试准备完成！");
    }
    ...
    public void examStart() {
        ...
        System.out.println("大学政治 模拟考试开始！");
        for (int i = 0; i < 8; ++i) {
            getValue(i);
        }
        sortScore();
