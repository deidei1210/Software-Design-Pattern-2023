...
public class SeaCraft extends Game implements Preparation {
    public SeaCraft(String Name) {
        super(Name);
    }

    @Override
    public void gamePreparation() {
        System.out.println("星河争霸 比赛准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("星河争霸 比赛准备完成！");
    }
    ...
    public void gameEnd() {
        ...
        setVisited(true);
        System.out.println("星河争霸 比赛完成！");
        System.out.println("星河争霸最终成绩：");
        for (int i = 1; i <= 8; ++i) {
    ...
    public void getValue(int no) {
        ...
        double value = (player.getHandspeed() * 1.5 + player.getReaction() * 1.2 + player.getIntelligence() * 1 + player.getEndurance() * 0.8+player.getLuck()*0.5) / player.getPlayerState().getState();
