...
public class ForwardWatch extends Game implements Preparation {
    public ForwardWatch(String Name) {
        super(Name);
    }

    @Override
    public void gamePreparation() {
        System.out.println("守望前锋：再也不来 比赛准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("守望前锋：再也不来 比赛准备完成！");
    }
    ...
    public void gameStart() {
        ...
        System.out.println("守望前锋：再也不来 比赛开始！");
        for (int i = 0; i < 8; ++i) {
            getValue(i);
        }
        sortScore();
    ...
    public void gameEnd() {
        ...
        System.out.println("守望前锋：再也不来 比赛完成！");
    ...
    public void getValue(int no) {
        ...
        double value = (player.getReaction() * 1.5 + player.getLuck() * 1.2 + player.getIntelligence() * 1 + player.getEndurance() * 0.8+player.getHandspeed()*0.5) / player.getPlayerState().getState();
