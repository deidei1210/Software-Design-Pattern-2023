...
public class LoreOfLU extends Game implements Preparation {
    public LoreOfLU(String Name) {
        super(Name);
    }

    @Override
    public void gamePreparation() {
        System.out.println("卢氏传说 比赛准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("卢氏传说 比赛准备完成！");
    }
    ...
    public void getValue(int no) {
        ...
        double value = (player.getLuck() * 1.5 + player.getIntelligence() * 1.2 + player.getReaction() * 1 + player.getEndurance() * 0.8+player.getHandspeed()*0.5) / player.getPlayerState().getState();
