...
public class HeroOfFan extends Game implements Preparation {
    public HeroOfFan(String Name) {
        super(Name);
    }

    @Override
    public void gamePreparation() {
        System.out.println("风扇英雄 比赛准备中...");
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        System.out.println("风扇英雄 比赛准备完成！");
    }
    ...
    public void getValue(int no) {
        ...
        double value = (player.getEndurance() * 1.5 + player.getIntelligence() * 1.2 + player.getHandspeed() * 1 + player.getReaction() * 0.8+player.getLuck()*0.5) / player.getPlayerState().getState();
